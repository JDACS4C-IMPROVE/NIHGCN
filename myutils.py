...
def cross_entropy_loss_masked(true_data: torch.Tensor, predict_data:  torch.Tensor):
    """
    :param true_data: true data
    :param predict_data: predict data
    :return: cross entropy loss
...
def evaluate_all(true_data: torch.Tensor, predict_data: torch.Tensor):
    ...
    loss = cross_entropy_loss_masked(true_data,predict_data)
    ...
    metrics = {'CrossEntropyLoss':loss.item(), 'auc':auc, 'ap':ap, 'acc':acc.item(), 'f1':f1.item(), 'mcc':mcc.item(), 'r2':r2}
    return metrics
